...
(deftest test-validation
  ...
  (vmatch tctx #{'myapp/SuperUser} {:name "niquola" :contact {:phone "888"}}
          {:errors
           [{:message ":ex is required",
             :type "require",
             :path [:contact :ex],
             :schema ['myapp/SuperUser :confirms 'myapp/Contactable :contact :require]}
            nil?]})
  ...
  (vmatch tctx #{'elcheck/Elcheck}
          {:patient {:genderCode "F",
                     ...
                     :birthDate "2017-01-01T00:00:00.000Z" #_"1970-01-01T00:00:00.000+0000",
                     :firstName "Nat",
                     :id "777",
                     :lastName "Color",
                     :gender "Female",
          ...
          {:errors
           [{:message "Expected type of 'map, got 'long"
            ...
            {:message ":system is required",
             ...
             :schema ['myapp/User :identifiers :every 1 :require]}
            {:type "unknown-key",
  ...
  (vmatch tctx #{'zen/schema} {:type 'zen/map :keys 1}
          {:errors
           [{:message "Expected type of 'map, got 'long"
             :type "map.type"
             :path [:keys]
             :schema ['zen/schema #_:schema-key #_'zen/map :keys :type]}]})
  ...
  (vmatch tctx #{'zen/schema} {:type 'zen/map :keys {:prop 1}}
          {:errors
           [{:message "Expected type of 'map, got 'long"
             :type "map.type"
             :path [:keys :prop]
             :schema ['zen/schema #_:schema-key #_'zen/map :keys :values :confirms 'zen/schema :type]}]})
  ...
  (vmatch tctx #{'zen/schema}
          ...
          {:errors
           [{:message "Expected type of 'integer, got 'string"
             ...
             :schema ['zen/schema #_:schema-key #_'zen/map :keys :values :confirms 'zen/schema #_:schema-key #_'zen/integer :min :type]}
            {:type "unknown-key",
             :message "unknown key :ups",
  ...
  (vmatch tctx #{'myapp/path}
          ...
          {:errors
           [{:message "Expected one of the cases to be true"
             :type "case"
             :path [2]
             :schema ['myapp/path :every 2 :case]}]})
  ...
  (vmatch tctx #{'myapp/const} 1
          {:errors
           [{:message "Expected type of 'string, got 'long",
             :type "string.type"
             :schema ['myapp/const :type]}

            {:message "Expected 'fixed', got '1'",
             :type "schema"
             :schema ['myapp/const :const]}]})
  ...
  (testing "zen/string"
    ...
    (invalid-schema tctx
                    ...
                    [{:message "Expected type of 'integer, got 'string",
                      :type "integer.type",
                      :path [:minLength],
                      :schema ['zen/schema #_:schema-key #_'zen/string :minLength :type]}

                     {:message "Expected type of 'integer, got 'double",
                      :type "integer.type",
                      :path [:maxLength],
                      :schema ['zen/schema #_:schema-key #_'zen/string :maxLength :type]}

                     {:message "Expected type of 'regex, got 'persistentarraymap",
                      :type "regex.type",
                      :path [:regex],
                      :schema ['zen/schema #_:schema-key #_'zen/string :regex :type]}])
...
;; schema-key
    ...
    ;; TODO fix this
    #_(valid-schema! tctx (get sk-sch 'obj))
  ...
  (testing "zen/symbol"
    ...
    (invalid-schema tctx
                    ...
                    [{:message "Expected type of 'set, got 'string",
                      :path [:tags],
                      :type "set.type",
                      :schema ['zen/schema :tags :confirms 'zen/tagset :type]}])
    ...
    (valid tctx 'test.enum-and-const/datetime "2015-02-07T13:28:17-05:00" #_"1994-09-26T16:40:00")
    ...
    (match tctx 'test.list/poly-list '(test.list/fn1 "str")
           [{:message "Expected type of 'integer, got 'string"
             :type "integer.type"
             :path [1]
             :schema ['test.list/poly-list :schema-index 'test.list/fn1 :nth 1 :type]}])
